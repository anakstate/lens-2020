...
def resultsSTACK():
    ...
    dim_steps = [(dim[step]/(step+1)) for step in steps]
...
def resultsFE_rnd(RULE):
    ...
    fe_labels = [(step+1)/(step+1) for step in steps]
...
def write_plot():
	...
	#	"    std_best = %r\n" % (best_mean_steps, best_std_steps))
        ...
        #str += ("    best = plt.plot(%r, avg_best, color='%s', label='BEST BP (auESC=%.4f)', linewidth=0.5)\n\n\n" % (x_ticks, colorbpfe[0], best_auc))

        
        fe2_labels, fe2_mean, fe_std, fe2_mean_steps, fe2_std_steps, fe2_auc = resultsFE_rnd('L2')
        str += ("    avg_fe2 = %r\n"
                "    std_fe2 = %r\n" % (fe2_labels, fe2_std_steps))
        str += ("    fe2 = plt.errorbar(%r, avg_fe2, yerr=std_fe2, color='%s', linewidth=0.5)\n" % (x_ticks, colorbpfe[1]))
        str += ("    fe2 = plt.plot(%r, avg_fe2, color='%s', label='FE-L2 (auESC=%.4f)', linewidth=1)\n\n\n" % (x_ticks, colorbpfe[1], fe2_auc))


        stack_labels, stack_mean, stack_std, stack_mean_steps, stack_std_steps, stack_auc = resultsSTACK()
        str += ("    dim_stack = %r\n"
                "    avg_stack = %r\n"
                "    std_stack = %r\n" % (stack_labels, stack_labels, stack_std_steps))
        if (annotation):
            str += ("    x=0\n"
		    "    for i,j in zip(%r, %s):\n"
                    "        plt.annotate(dim_stack[x], xy=(i,j+0.001), fontsize = 5, color='black')\n"
                    "        x+=1\n" % (x_ticks, stack_mean_steps))
        str += ("    stack = plt.errorbar(%r, avg_stack, yerr=std_stack, color='black', linewidth=0.5)\n" % (x_ticks))
        str += ("    stack = plt.plot(%r, avg_stack, color='black', label='FE-L1 (STACKING) (auESC=%.4f)', linewidth=1)\n\n\n" % (x_ticks, stack_auc))
...
#			    "        x+=1\n" % (x_ticks, (16-2*index), color[index]))
                ...
                str += ("    rl = plt.plot(%r, avg_rl, color=\'%s\', label=\'RL_%s_conv%s (auESC=%.4f)\', linewidth=1)\n\n\n" % (x_ticks, color[index], strategy, conv, rl_auc))
		...
		#str += ("    rl = plt.errorbar(%r, avg_rl, yerr=std_rl, color=\'%s\', linewidth=0.5, linestyle = '--')\n" % (x_ticks, color[index]))
                #str += ("    rl = plt.plot(%r, avg_rl, color=\'%s\', label=\'RL_%s_conv%s (auESC=%.4f)\', linewidth=1, linestyle = '--')\n\n\n" % (x_ticks, color[index], strategy, conv, rl_auc))
                index += 1
                ...
                str += ("    dim_rl = %r\n"
                        "    avg_rl = %r\n"
                        "    std_rl = %r\n" % (rl_labels, rl_labels, rl_std_steps))
                ...
                if (annotation):
                    str += ("    x=0\n"
                            ...
                            "        x+=1\n" % (x_ticks, color[index]))
                str += ("    rl = plt.errorbar(%r, avg_rl, yerr=std_rl, color=\'%s\', linewidth=0.5)\n" % (x_ticks, color[index]))
                str += ("    rl = plt.plot(%r, avg_rl, color=\'%s\', label=\'%s (auESC=%.4f)\', linewidth=1)\n\n\n" % (x_ticks, color[index], strategy, rl_auc))
                index += 1
            else:
                conv = '10'
                ...
                if (annotation):
                    str += ("    x=0\n"
                            ...
                            "        x+=1\n" % (x_ticks, color[index]))
                str += ("    rl = plt.errorbar(%r, avg_rl, yerr=std_rl, color=\'%s\', linewidth=0.5)\n" % (x_ticks, color[index]))
                str += ("    rl = plt.plot(%r, avg_rl, color=\'%s\', label=\'RL_%s_conv10 (auESC=%.4f)\', linewidth=1)\n\n\n" % (x_ticks, color[index], strategy, rl_auc))	
                index += 1
        str += ("    plt.title('%s -- epsilon=%s, size curves')\n" % (proj, epsilon))
        ...
        str += ("    plt.yticks(yticks, fontsize=15)\n")
        str += ("    plt.ylabel('Size', fontsize=20)\n")
...
def generate_script(project_path):
    header = ("from numpy import nan\n"
              ...
              "with PdfPages('%s_auESizeC.pdf') as pdf:\n\n    plt.style.use('ggplot')\n" % (proj))
    script_name = "%s/PLOTS/%s_auESizeC.py" % (project_path, proj)
